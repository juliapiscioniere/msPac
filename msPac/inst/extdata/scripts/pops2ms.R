...
pops2ms_new = function(pops, migmat, texp, nloci, foundN = 1, popsizes = 1000, popsamps = 100, mutrate = 1e-8, frag.length = 80, island=NULL) 
{
  if(length(popsizes) == 1) {
    popsizevec = rep(popsizes, dim(pops)[1]) 
  } else {
...
call_ms = function(nsamp, nrep, t, I, migmat, en_matrix, ej_matrix){
  seeds = round(runif(3,min=1,max=100000))
  migr = 0
