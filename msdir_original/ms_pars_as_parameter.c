...
main(int argc, char *argv[])
{
	...
	struct params getpars( int argc, char *argv[], int *howmany, struct params pars)  ;
	int gensam( char **list, double *probss, double *ptmrca, double *pttot, struct params pars) ;
    struct params pars ;
	...
	pars = getpars( argc, argv, &howmany, pars) ;   /* results are stored in global variable, pars */ //NEED IT TO RETURN PARS???
    ...
    while( howmany-count++ ) {
	   if( (ntbs > 0) && (count >1 ) ){
			 ...
			 pars = getpars( argc, argv, &howmany, pars) ;
	   }
	   ...
	   if( ntbs >0 ){
		...
		printf("\n");
        segsites = gensam( list, &probss, &tmrca, &ttot, pars ) ;
	...
	int 
gensam( char **list, double *pprobss, double *ptmrca, double *pttot, struct params pars )
{
...
struct params getpars(int argc, char *argv[], int *phowmany, struct params pars)
{
