...
pops2ms = function(pops, migmat, texp, nloci, foundN = 1, popsizes = 1000, popsamps = 100, mutrate = 1e-8, frag.length = 80, msfile = "msline.txt", simfile = "msout.out", island=NULL) 
{
...
{                                        #MIGMAT
	...
	jumpline = paste(jumpline, sep = " ", collapse = " ")
        ...
        parline = paste("-t", THETA*10000, "-I", NPOP, paste(popsampvec, sep = " ", collapse = " "), migvec, sizeline, jumpline, "-T")

	    #parline1 = paste("-t", THETA*10000)
	    #parline2 = paste("-I", NPOP, paste(popsampvec, sep = " ", collapse = " "))
	    #parline3 = paste(migvec, sizeline, jumpline, "-T")
        
        #cat(file="tmpfile", parline1, parline2, parline3,"\n")
        cat(file="tmpfile", parline,"\n")
###
###        trees <- ms(sum(popsampvec),nloci,opts=parline)
###        seqs <- lapply(trees$trees,function(tree)
###        {
###            seqgen(opts='-mGTR -r 1 1 1 1 1 1 -l 80',newick.tree=tree)
###        })

                                        #seqs
        list(progline, parline)
        #list(progline,parline1, parline2, parline3)
}
